...
bool LScene::initShader(const SceneInitInfo&)
{
	...
	std::string vs = get_shader_BasePath() + "texture/skybox.vert";
	std::string fs = get_shader_BasePath() + "texture/skybox.frag";
...
void LScene::render(PassInfo&ps)
{
	...
	location = currentShader->getVariable("view");
	if (location != -1)
		currentShader->setMatrix4(location, 1, GL_FALSE, math::value_ptr(glm::mat4(glm::mat3(camera_->getViewMatrix()))));
