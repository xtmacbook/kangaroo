...
int main(int argc, const char* argv[]) {
	...
	settings.window.defaultIconFilename = "D:/workspace/engines/g3d/G3D10/data-files/icon/icon.png";
...
#   endif
	...
	alwaysAssertM(notNull(m_framebuffer->texture(Framebuffer::DEPTH)), "GApp failed to allocate a depth buffer");
...
void App::loadScene() {
	...
	const String& sceneName = m_sceneDropDownList->selectedValue().text();
...
void App::selectEntity(const shared_ptr< Entity >& e) {
	m_selectedEntity = e;

	if (notNull(m_selectedEntity)) {
		const shared_ptr< Entity::Track> tr = e->track();
		const shared_ptr<Entity::SplineTrack> str = dynamic_pointer_cast<Entity::SplineTrack>(tr);
		m_splineEditor->setSpline(str->spline());
...
void App::onGraphics3D(RenderDevice* rd, Array<shared_ptr<Surface> >& allSurfaces) {
	...
	// Compute AO
	LightingEnvironment& le = m_scene->lightingEnvironment();
	m_ambientOcclusion->update(rd,le.ambientOcclusionSettings, defaultCamera, m_framebuffer->texture(Framebuffer::DEPTH));

	// Compute shadow maps and forward-render visible surfaces
	le.ambientOcclusion = m_ambientOcclusion;
	
	//G3D::Renderer::render(rd, defaultCamera, m_framebuffer, allSurfaces);
	...
	if (m_showLightSources) {
		//Draw::light(m_scene->lighting(), rd);
	}

	// Call to make the GApp show the output of debugDraw
	drawDebugShapes();

	m_depthOfField->apply(rd, m_framebuffer->texture(Framebuffer::COLOR0), m_framebuffer->texture(Framebuffer::DEPTH), defaultCamera, m_debugOption);
}
