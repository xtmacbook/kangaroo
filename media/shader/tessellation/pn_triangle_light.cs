...
struct OutputPatch
 ...
 //the builtin keyword patch says that the variable contains data which pertains to the entire patch and
 // not the current output CP. The compiler can use that as a hint to make sure that the code 
 //that updates such a variable will run once per patch instead of once per CP (since GPUs will strive to update each output CP in a different HW thread).
