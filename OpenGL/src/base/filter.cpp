...
{
    x = fabsf(x);
    if( x < 3.0f ) return sincf(math::Const<float>::pi() * x) * sincf(math::Const<float>::pi() * x / 3.0f);
    return 0.0f;
}
...
{
    return sincf(math::Const<float>::pi() * x);
}
...
{
    const float sinc_value = sincf(math::Const<float>::pi() * x * stretch);
...
{
    // variance = sigma^2
    return (1.0f / sqrtf(2 * math::Const<float>::pi() * variance)) * expf(-x*x / (2 * variance));
}
...
Kernel2::Kernel2(math::uint ws) : m_windowSize(ws)
{
    m_data = new float[m_windowSize * m_windowSize];
}

Kernel2::Kernel2(math::uint ws, const float * data) : m_windowSize(ws)
{
    m_data = new float[m_windowSize * m_windowSize];
...
{
    m_data = new float[m_windowSize * m_windowSize];
    for (math::uint i = 0; i < m_windowSize * m_windowSize; i++) {
        m_data[i] = k.m_data[i];
    }
}
...
{
    for(math::uint i = 0; i < m_windowSize; i++) {
        for(math::uint j = i+1; j < m_windowSize; j++) {
...
void Kernel2::initBlendedSobel(const math::V4f & scale)
{
...
PolyphaseKernel::PolyphaseKernel(const Filter & f, math::uint srcLength, math::uint dstLength, int samples/*= 32*/)
{
    ...
    for (math::uint i = 0; i < m_length; i++)
    {
