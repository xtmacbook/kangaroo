...
void PBuffer::parseModeString(const char *modeString, vector<int> *pfAttribList, vector<int> *pbAttribList)
{
	...
	while (buf != NULL)
	{
		...
		else if (strstr(buf, "arb_float") != NULL)
			bIsARBFloatBuffer = true;
	...
	else if (bIsARBFloatBuffer) {
#ifdef WGL_ARB_pixel_format_float
		pfAttribList->push_back(WGL_TYPE_RGBA_FLOAT_ARB);
#endif
	}
	else {
	...
	for (unsigned int i = 0; i < tokens.size(); i++)
	{
		...
		if ((token.find("ati_float") == 0) || (token.find("arb_float") == 0))
		{
		...
		if (token.find("texture") == 0)
		{
			...
			if (bIsFloatBuffer || bIsATIFloatBuffer || bIsARBFloatBuffer)
			{
				if (m_iNComponents == 0)
				{
